...
def read_dataset():
    ...
    :return:
    """
    data_dir = pathlib.Path('car_datasets/digit_letters')
...
def one_hot(labels):
    ...
    :return:
    """
    onehot_labels = np.zeros(shape=[len(labels), 36])
...
def cnn_net(input_shape):
    ...
    model.add(keras.layers.Dense(units=36, activation=tf.nn.softmax))
    print(model.summary())
    return model
...
def train_model(train_images, train_labels, test_images, test_labels):
    ...
    y_pred, y_test = [], []
    TP, TN, FP, FN = np.zeros(36), np.zeros(36), np.zeros(36), np.zeros(36)
    ...
    model.save('model/digit_letters-model.h5')

    l = []
    for path in pathlib.Path('car_datasets/digit_letters').iterdir():
